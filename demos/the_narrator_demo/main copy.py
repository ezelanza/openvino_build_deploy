...
def load_llava_video_models(model_name_video: str, device: str = "CPU") -> tuple:
    """Load LLaVA-NeXT-Video model with Intel HF Optimum (OpenVINO)"""
    model_dir = MODEL_DIR / model_name_video.replace("/", "_")
    
    # Download and convert if not exists
    if not model_dir.exists():
        download_and_convert_llava_video(model_name_video)
    ...
    model_video = OVModelForVisualCausalLM.from_pretrained(
        str(model_dir),
    ...
    return model_video, processor

def download_and_convert_llava_video(model_name_video: str) -> None:
    ...
    # Save processor
    processor = LlavaNextVideoProcessor.from_pretrained(model_name_video)
    processor.save_pretrained(output_dir)
...
def run(video_path: str, model_name: str, flip: bool = True, video_input: bool = False) -> None:
    ...
    model_video, processor_video = load_llava_video_models(model_name_video, device_type)
